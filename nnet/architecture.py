...
class DoseModel(object):
    def __init__(self, opts, is_training=False):
        self.opts = opts
        self.true_images = tf.placeholder(tf.float32, [None, 3, 256, 256], "images")
        self.input_ptv = tf.placeholder(tf.float32, [None, 3, 256, 256], "input_ptv")
        self.input_oct = tf.placeholder(tf.float32, [None, 6, 256, 256], "input_ptv")
        ...
        self.summary_image = tf.transpose(self.predicted_imgs, perm=[0, 2, 3, 1])
        tf.summary.image('Predicted image', self.summary_image, max_outputs=4)
    ...
    def create_network(self):
        ...
        antires_block4 = model.block_layer(antires_block3, 64, 3, 2, self.is_training, "antires_block4", data_format='channels_first', transpose=True)     
        # print (ptv_conv2)
        ...
        # antires_endconv = tf.nn.sigmoid(antires_endconv)
    ...
    def train(self):
        ...
        for epoch in range(1, self.opts.num_epochs):
            ...
            for batch_begin, batch_end in zip(range(0, train_size, self.opts.batch_size), \
                ...
                batch_num += 1
                self.writer.add_summary(summary, epoch * (train_size/self.opts.batch_size) + batch_num)
                if batch_num % self.opts.display == 0:
                    rem_time = (time.time() - begin_time) * (self.opts.num_epochs-epoch) * (train_size/self.opts.batch_size)
                    log  = '-'*20
                    log += ' Epoch: {}/{}|'.format(epoch, self.opts.num_epochs)
                    log += ' Batch Number: {}/{}|'.format(batch_num, train_size/self.opts.batch_size)
                ...
                if epoch % self.opts.ckpt_frq == 0 and batch_num == 1:
                    self.saver.save(self.sess, "ckpt/"+"{}_{}_{}".format(epoch, lr, loss))